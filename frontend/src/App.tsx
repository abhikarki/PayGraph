...
function App() {
  ...
  return (
    <div className="app-wrapper">
      ...
      <main className={`app-content ${selectedPair ? 'with-panel' : ''}`}>
            ...
            <div className="panel-content">
              ...
              <section className="panel-section">
                ...
                <div className="token-info">
                  <h4>{selectedPair.token0}</h4>
                  {selectedPair.api_response?.token0 && (
                    <>
                      <div className="info-row small">
                        <label>Address:</label>
                        <span className="address">{selectedPair.api_response.token0.address?.slice(0, 12)}...</span>
                      </div>
                      <div className="info-row small">
                ...
                <div className="token-info">
                  <h4>{selectedPair.token1}</h4>
                  {selectedPair.api_response?.token1 && (
                    <>
                      <div className="info-row small">
                        <label>Address:</label>
                        <span className="address">{selectedPair.api_response.token1.address?.slice(0, 12)}...</span>
                      </div>
                      <div className="info-row small">
                        <label>Decimals:</label>
              ...
              {selectedPair.api_response?.pairAddress && (
                <section className="panel-section">
                  <h3>Pair Address</h3>
                  <div className="info-row">
                    <span className="address">{selectedPair.api_response.pairAddress}</span>
                  </div>
                </section>
              )}
            </div>
